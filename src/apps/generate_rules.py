...
def generate_rules(config: Dict):
    dataset = load_dataset_from_jsonl(config.get('dataset_path'))

    with open(config.get('save_path'), 'w+') as fout:
...
# python -m src.apps.generate_rules --path config/generate_rules.yaml
if __name__ == "__main__":
    config = Config.parse_args_and_get_config()
    generate_rules(config.get('generate_rules_app'))
