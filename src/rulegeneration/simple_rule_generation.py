...
def word_rule(data: Dict) -> AstNode:
    if data['e1_start'] > data['e2_start']:
        return parse_surface(' '.join([f"""[word="\{x}"]""" for x in data['tokens'][data['e2_end']:data['e1_start']]]))
    else:
        return parse_surface(' '.join([f"""[word="\{x}"]""" for x in data['tokens'][data['e1_end']:data['e2_start']]]))
...
if __name__ == "__main__":
    ...
    print(word_rule(data1).pattern)
    print(word_rule(data2).pattern)
    print(word_rule(data3).pattern)
    print(word_rule(data4).pattern)
